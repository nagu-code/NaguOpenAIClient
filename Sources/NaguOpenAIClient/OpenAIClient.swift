...
public struct OpenAIClient {
    ...
    public var convertation: MessageList = MessageList(messages: [])
    
    public init(apiKey: String, serverURL: URL? = nil) {
        let resolvedServerURL = serverURL ?? (try! Servers.server1())
        self.client = Client(
            serverURL: resolvedServerURL,
            transport: URLSessionTransport(),
            middlewares: [AuthMiddleware(apiKey: apiKey)])
        self.apiKey = apiKey
        self.serverURL = resolvedServerURL
    }
    ...
    public mutating func promptChatSW(message: String) async throws -> (text: String?, imageURL: String?) {
        ...
        convertation.messages.append(MessageData(role: "user", content: message))

        let jsonData = try JSONEncoder().encode(convertation.self)
    ...
    /// Use URLSession manually until swift-openapi-runtime support MultipartForm
    public func generateAudioTransciptions(audioData: Data, fileName: String = "recording.m4a", serverURL: URL? = nil) async throws -> String {
        let defaultURLString = "https://api.openai.com/v1/audio/transcriptions"
        let resolvedURL = serverURL ?? URL(string: defaultURLString)!
